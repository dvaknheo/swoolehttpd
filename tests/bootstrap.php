...
{
    $ret=__DIR__.'/data_for_tests'.str_replace(['SwooleHttpd\\','\\'],['/','/'],$class).'/';
    return $ret;
}
...
class MyCodeCoverage
{
    ...
    {
var_dump("zzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzz");
var_dump($path);
        if (is_file($path)) {
    ...
    public function end()
    {
        ...
        $path=substr(str_replace('\\', '/', $this->test_class), strlen('SwooleHttpd\\'));
...
class TestFileGenerator
{
    ...
    protected static function MakeTest($file, $short_file)
    {
        ...
        $ns='SwooleHttpd\\'.str_replace('/', '\\', dirname($short_file));
        $ns=str_replace('\.', '', $ns);
